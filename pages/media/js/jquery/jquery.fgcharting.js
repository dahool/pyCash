/**
 * --------------------------------------------------------------------
 * jQuery-Plugin "fgCharting" by Scott Jehl, scott@filamentgroup.com
 ...
 * 
 * Copyright (c) 2008 Filament Group, Inc Licensed under GPL
 * (http://www.opensource.org/licenses/gpl-license.php)
 * 
 * Usage Notes: please refer to our article above for documentation
 * 
 * Version: 1.0, 11.11.2007 Changelog:
 * 
 * --------------------------------------------------------------------
 */
...
$.fn.createLineGraph = function(tableData, filledLine) {
	...
	if ($(this).parents('.chartBlock').size() == 0)
		$(this)
				.wrap('<div class="chartBlock" style="position: relative;"></div>');
...
$.fn.createAdditiveLineGraph = function(tableData, filledLine) {
	...
	$('#' + ulID).remove();
	$(this).after('<ul id="' + ulID + '"></ul>');
	$(yLabelsAdditive).each(function(i) {
		$('#' + ulID).prepend('<li>' + this + '</li>');
	});
	$('#' + ulID + ' li').css({
		'padding' : 0,
	...
	for (var h = 0; h < members.length; h++) {
		...
		for (var i = 0; i < points.length; i++) {
			...
			if (nextPrevPoints[i])
				nextPrevPoint = nextPrevPoints[i];
			ctx.lineTo(integer, Math
					.round((-points[i] - prevPoint - nextPrevPoint) * yScale));
			integer += xInterval;
		}
		ctx.strokeStyle = tableData.members()[h].color;
		ctx.stroke();
...
// Credit: Some of the ideas for this pie chart plotting were borrowed from
...
$.fn.createPieChart = function(tableData) {
	...
	$(this)
			.after('<ul id="'
					+ ulID
					+ '" style="position:absolute; list-style: none; top: -2.5em; left: -3em;"></ul>');
	...
	$(memberTotals).each(function(i) {
		...
		ctx.arc(centerx, centery, radius,
				counter * Math.PI * 2 - Math.PI * 0.5, (counter + fraction)
						* Math.PI * 2 - Math.PI * 0.5, false);
		...
		var labelx = centerx + Math.sin(sliceMiddle * Math.PI * 2)
				* (radius / 2);
		var labely = centery - Math.cos(sliceMiddle * Math.PI * 2)
				* (radius / 2);

		$('#' + ulID)
				.append('<li style="color:#fff; list-style: none !important; font-size: 1.1em; font-weight: bold; position: absolute; left:'
						+ labelx
						/ 10
						+ 'em; top:'
						+ labely
...
$.fn.createBarGraph = function(tableData) {
	...
	for (var h = 0; h < members.length; h++) {
		...
		for (var i = 0; i < points.length; i++) {
			...
			ctx.lineTo(Math.round(integer + (h * linewidth)), Math
					.round(-points[i] * yScale));
			integer += xInterval;
		}
...
$.fn.getTableData = function(chartDimensions) {
	var tableObj = this;
	var colors = ['#be1e2d', '#666699', '#92d5ea', '#ee8310', '#8d10ee',
			'#5a3b16', '#26a4ed', '#f45a90', '#e9e744'];
	var tableData = {
		members : function() {
			var members = [];
		...
		topValue : function() {
			...
			var allData = this.allData().join(',').split(',');
			$(allData).each(function() {
				if (parseInt(this) > topValue)
					topValue = parseInt(this);
			});
			return topValue;
		},
		...
		topYtotal : function() {
			...
			$(yTotals).each(function() {
				if (parseInt(this) > topYtotal)
					topYtotal = parseInt(this);
			});
			return topYtotal;
		},
		xLabels : function() {
...
$.fgCharting = function() {
	$('[class^=fgCharting_]').each(function() {
		// get the class that contains fgCharting settings and set it to
		// chartClass var
		var thisClass = $(this).attr('class');
		var chartClass = '';
		thisClass = thisClass.split(' ');
		$(thisClass).each(function() {
			if (this.match('fgCharting_'))
				chartClass = this;
		});
		...
		$(chartClass).each(function() {
			if (this.match('src-'))
				chartSrc = '#' + this.split('-')[1];
			if (this.match('type-'))
				chartType = this.split('-')[1];
		});
		...
		if ($(chartSrc).size() > 0 && chartType != '') {
			...
			switch (chartType) {
				case 'line' :
					$(this).createLineGraph(tableData);
					break
				case 'filledLine' :
					$(this).createLineGraph(tableData, true);
					break
				case 'additiveLine' :
					$(this).createAdditiveLineGraph(tableData);
					break
				case 'additiveFilledLine' :
					$(this).createAdditiveLineGraph(tableData, true);
					break
				case 'pie' :
					$(this).createPieChart(tableData);
					break
				case 'bar' :
					$(this).createBarGraph(tableData);
					break
				case 'additiveBar' :
					$(this).createAdditiveBarGraph(tableData);
					break
			}
		}
	});
