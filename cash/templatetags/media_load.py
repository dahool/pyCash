...
def loadMedia(parser, token):
    ...
    line = '<link rel="stylesheet" href="%s%s"/>' % (settings.STATIC_URL,bits[1])
    return MediaNode(line)
loadMedia = register.tag(loadMedia)

def loadStyle(parser, token):
    bits = token.contents.split('"')
    line = '<link rel="stylesheet" href="%s%s"/>' % (settings.STATIC_URL + "css/",bits[1])
    return MediaNode(line)
...
def loadExtStyle(parser, token):
    bits = token.contents.split('"')
    line = '<link rel="stylesheet" href="%s%s"/>' % (settings.STATIC_URL + settings.EXT_LOCATION + 'resources/css/',bits[1])
    return MediaNode(line)
...
def loadUxStyle(parser, token):
    bits = token.contents.split('"')
    line = '<link rel="stylesheet" href="%s%s"/>' % (settings.STATIC_URL + settings.UX_LOCATION + 'css/',bits[1])
    return MediaNode(line)
...
def loadScript(parser, token):
    bits = token.contents.split('"')
    line = '<script type="text/javascript" src="%s%s"></script>' % (settings.STATIC_URL,bits[1])
    return MediaNode(line)    
...
def loadExtUx(parser, token):
    bits = token.contents.split('"')
    line = '<script type="text/javascript" src="%(url)s%(location)s%(file)s.js"></script>' % {'url':settings.STATIC_URL,'location': settings.UX_LOCATION,'file':bits[1]}
    return MediaNode(line)    
